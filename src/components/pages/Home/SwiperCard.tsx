...
export const SwiperCard: React.FC<WidgetNewsData> = ({ title, photoURL, smallText, id }) => {
  return (
    <div className='flex h-full max-w-full rounded-lg bg-light-blue s:flex-col sm:min-h-[368px] lg:flex-row'>
      <div className='card bg-base-100 shadow-xl s:w-full lg:w-3/5'>
        <img
          src={photoURL}
          alt='Shoes'
          className='h-full w-full object-cover s:rounded-tl-lg s:rounded-tr-lg lg:rounded-bl-lg  lg:rounded-tr-none'
        />
      </div>

      <div className='relative flex max-h-[388px] grow  flex-col  p-10 s:w-full  lg:w-[56%] xl:w-2/5'>
